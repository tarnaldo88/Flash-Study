...
public static class SeedData
{
    public static async Task EnsureGoDeckAsync(SQLiteAsyncConnection db)
    {
        ...
        var cards = new List<Card>
        {
            ...
            new() { DeckId = deck.Id, Front = "What is GOPATH?", Back = "Legacy workspace path for Go projects; less used with modules.", CreatedAt = now, UpdatedAt = now },
            ...
            new() { DeckId = deck.Id, Front = "What is JSON marshaling?", Back = "Encoding structs to JSON using encoding/json.", CreatedAt = now, UpdatedAt = now },
            ...
            new() { DeckId = deck.Id, Front = "What is net/http used for?", Back = "Building HTTP clients and servers.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "How to create a basic HTTP server?", Back = "Use http.HandleFunc and http.ListenAndServe.", CreatedAt = now, UpdatedAt = now },
            ...
            new() { DeckId = deck.Id, Front = "What is cgo?", Back = "A tool for calling C code from Go.", CreatedAt = now, UpdatedAt = now },
            ...
            new() { DeckId = deck.Id, Front = "What is Go's GC?", Back = "Concurrent garbage collector managing heap memory.", CreatedAt = now, UpdatedAt = now },
    ...
    public static async Task EnsureSoftwareEngineerDeckAsync(SQLiteAsyncConnection db)
    {
        ...
        var cards = new List<Card>
        {
            ...
            new() { DeckId = deck.Id, Front = "What is a priority queue?", Back = "An ADT where items are removed by priority, often implemented with a heap.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is breadth-first search (BFS)?", Back = "Graph traversal visiting neighbors level by level.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is depth-first search (DFS)?", Back = "Graph traversal exploring as far as possible before backtracking.", CreatedAt = now, UpdatedAt = now },
            ...
            new() { DeckId = deck.Id, Front = "What is topological sort used for?", Back = "Ordering nodes in a DAG based on dependencies.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is a DAG?", Back = "Directed acyclic graph with no cycles.", CreatedAt = now, UpdatedAt = now },
            ...
            new() { DeckId = deck.Id, Front = "What is A* search?", Back = "Shortest-path search using a heuristic to guide exploration.", CreatedAt = now, UpdatedAt = now },
            ...
            new() { DeckId = deck.Id, Front = "What is context switching?", Back = "Saving and restoring CPU state to switch between threads/processes.", CreatedAt = now, UpdatedAt = now },
            ...
            new() { DeckId = deck.Id, Front = "TCP vs UDP.", Back = "TCP is reliable/ordered; UDP is faster but unordered/unreliable.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is the TCP three-way handshake?", Back = "SYN, SYN-ACK, ACK to establish a connection.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is DNS?", Back = "System that maps domain names to IP addresses.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is an HTTP header?", Back = "Metadata sent with an HTTP request or response.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is HTTP/2 main benefit?", Back = "Multiplexing many requests over a single connection.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is GraphQL?", Back = "Query language for APIs allowing clients to request specific data.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is gRPC?", Back = "High-performance RPC framework using Protobuf and HTTP/2.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "JSON vs Protobuf.", Back = "JSON is human-readable; Protobuf is compact and faster to parse.", CreatedAt = now, UpdatedAt = now },
            ...
            new() { DeckId = deck.Id, Front = "PUT vs POST.", Back = "PUT is idempotent create/replace; POST creates with server-assigned id.", CreatedAt = now, UpdatedAt = now },
            ...
            new() { DeckId = deck.Id, Front = "What is CORS?", Back = "Browser security rule controlling cross-origin requests.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is OAuth 2.0?", Back = "Authorization framework for delegated access.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is a JWT?", Back = "A signed JSON token used for claims and authentication.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is TLS?", Back = "Protocol that encrypts data in transit.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is SQL injection?", Back = "Attack that injects SQL through unsanitized inputs.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is XSS?", Back = "Cross-site scripting: injecting scripts into web pages.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is CSRF?", Back = "Cross-site request forgery using a victim's session.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is RBAC?", Back = "Role-based access control for permissions.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is SSO?", Back = "Single sign-on across multiple apps.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "CI vs CD.", Back = "CI automates builds/tests; CD automates delivery or deployment.", CreatedAt = now, UpdatedAt = now },
    ...
    public static async Task EnsureSystemDesignDeckAsync(SQLiteAsyncConnection db)
    {
        ...
        var cards = new List<Card>
        {
            ...
            new() { DeckId = deck.Id, Front = "What is write-behind caching?", Back = "Writes are buffered and persisted to the DB asynchronously.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "What is cache stampede?", Back = "Many requests miss cache at once and overload the DB.", CreatedAt = now, UpdatedAt = now },
            new() { DeckId = deck.Id, Front = "Cache eviction strategies.", Back = "LRU, LFU, FIFO, TTL-based eviction.", CreatedAt = now, UpdatedAt = now },
            ...
            new() { DeckId = deck.Id, Front = "What is API rate limiting by token bucket?", Back = "Tokens refill at a rate; requests consume tokens.", CreatedAt = now, UpdatedAt = now },
            ...
            new() { DeckId = deck.Id, Front = "What is ETL?", Back = "Extract, Transform, Load process for moving data.", CreatedAt = now, UpdatedAt = now },
            ...
            new() { DeckId = deck.Id, Front = "What is SSO vs federation?", Back = "SSO lets users sign in once; federation trusts external identity providers.", CreatedAt = now, UpdatedAt = now }
        };
